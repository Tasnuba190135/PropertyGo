...
class FileManager
{
    ...
    /**
     * setValue file details by file_id and status (default: 1).
     ...
     */
    public function setValueById($file_id, $status = 1)
    {
    ...
    /**
     * setValue files by file_owner_id and status.
     * If exactly one row is found, set the class properties accordingly (like setValueById()).
     ...
     * @return array|bool|int Returns 1 if one row is set into class properties, an array of rows if multiple are found, or false if none are found.
     */
    public function setValueByOwner($file_owner_id, $status = null)
    {
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            if (mysqli_num_rows($result) == 1) {
                ...
                // Set class properties like in setValueById()
                $this->file_id = $row['file_id'];
                $this->status = $row['status'];
                $this->file_owner_id = $row['file_owner_id'];
                $this->file_original_name = $row['file_original_name'];
