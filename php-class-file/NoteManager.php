...
class NoteManager
{
    ...
    public function ensureConnection()
    {
        if (!$this->conn) { // Check if connection is not set
            $db = new DbConnector(); // Create DB Connection
            $db->connect();
    ...
    public function alterTableAddColumns($selectedNums = null)
    {
        ...
        $alterQueries = [
            ...
            5 => ['updated', "ALTER TABLE $table ADD COLUMN updated TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP"]
        ];
    ...
    /**
     * setValue notes by note_id and status. If exactly one row is found, set class properties.
     * @param int|null $note_id (Optional) Specific note_id to set
     * @param int|null $status (Optional) Status filter
     * @return array|false Returns an array of results or false if no match
     */
    public function setValueByNoteId($note_id = null, $status = null)
    {
    ...
    /**
     * setValue notes based on owner_id and status. If exactly one row is found, set class properties.
     * @param int $owner_id Owner ID to search for
     * @param int|null $status (Optional) Status filter
     * @return array|false Returns an array of results or false if no match
     */
    public function setValueByOwnerId($owner_id = null, $status = null)
    {
